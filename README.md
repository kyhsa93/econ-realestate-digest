...
- **옛 서비스워커 때문에 실패하면 페이지가 한 번은 스스로 복구한다.** 예전 규칙은 캐시 미스에서
  응답을 아예 안 줘서, 파일이 멀쩡해도 새로고침할 때마다 실패했다. 사용자가 개발자도구를 열 이유는
  없으니 실패 시 `registration.update()`를 부르고 한 번 다시 받아본다. 등록할 때 `updateViaCache: "none"`을
  주는 것도 같은 이유다 — 안 주면 `sw.js` 자체가 HTTP 캐시에서 나와 고친 규칙이 최대 하루 동안 안 닿는다
- **로드가 한 번 실패하면 자동으로 한 번 더 받아본다.** 넷 중 하나가 순간적으로 실패한 것뿐인데
  사용자가 버튼을 눌러야만 복구되는 건 잘못이다. **오류 배너는 플래그가 아니라 "지금 화면에 실제로
  빠진 데이터"를 기준으로 띄운다** — 플래그로 띄우면 잠깐 실패했다 복구돼도 배너가 남아서, 데이터가
  멀쩡한데 고장난 것처럼 보인다(실제로 그 제보를 받았다)
